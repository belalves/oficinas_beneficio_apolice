...
| **Atualizações no sistema** *(painel principal)* | Um único bloco que **calcula a distância real** (raio de 50 km) entre cada município do Brasil e as oficinas ativas e mostra, por estado, o que **incluir** (município no raio ainda fora do sistema) e o que **retirar** (cidade do sistema fora do raio). O que aparece na tela é **exatamente** o que sai na planilha. Botões: **↻ Recalcular** e **⬇️ Baixar planilha (.xlsx)**. |
| **Baixar planilha (.xlsx)** | Devolve a **mesma planilha** com a aba **Sistemas** atualizada (colunas **Incluir**/**Retirar** no formato `DD/MM - Cidade (UF)`) e uma aba **Conferência** — uma cidade por linha, com distância (km) até a oficina mais próxima, nome da oficina, situação e ação sugerida (ótima para auditar/ordenar no Excel). A aba **Regiões não é alterada**. |
| **Cidades elegíveis** | Lista, por estado, todos os municípios dentro do raio de 50 km de alguma oficina ativa. |
